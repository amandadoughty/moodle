...
/**
 * Helper functions for CUL Course Dashboard
 *
 * @package    block
 * @subpackage culcourse_dashboard
...
/**
 * Returns an array of users with the role, for a course
 *
 * @param int $cid
 * @param int $roleid
 * @return array $roleusers
 */

function block_culcourse_dashboard_get_usersbyrole($cid,$roleid) {
    $context  = context_course::instance($cid, IGNORE_MISSING);
    $roleusers = get_role_users($roleid, $context, false, 'u.id, u.username, u.firstname, u.lastname');
    return $roleusers;
}


/**
 * block_culcourse_dashboard_get_reading_list_url_data()
 *
 * @param mixed $course
 * @return
 */
function block_culcourse_dashboard_get_reading_list_url_data($course) {
    global $CFG;

    $site = trim(get_config('aspirelists', 'targetAspire')); // 1.x: $CFG->block_aspirelists_targetAspire;
    $codedata = block_culcourse_dashboard_get_coursecode_data($course);
    ...
    $connectiontimeout = trim(get_config('culcourse_dashboard', 'connection_timeout'));
    $transfertimeout   = trim(get_config('culcourse_dashboard ', 'transfer_timeout'));

    $data = block_culcourse_dashboard_get_reading_list_data($path, $url, $connectiontimeout, $transfertimeout);
    ...
    if (OK == $data['status']) {
        ...
        $data = block_culcourse_dashboard_get_reading_list_data($path, "{$path}.json", $connectiontimeout, $transfertimeout);
        if (OK === $data['status']) {
            return array('status' => OK, 'listtype' => 'module', 'url' => $path);
        }
    }
...
/**
 * block_culcourse_dashboard_get_reading_list_data()
 *
 ...
 */
function block_culcourse_dashboard_get_reading_list_data($path, $url, $connectiontimeout = null, $transfertimeout = null) {
...
/**
 * block_culcourse_dashboard_get_timetable_url()
 *
 * @param mixed $course
...
/**
 * block_culcourse_dashboard_get_photoboard_url()
 *
 * @param mixed $course
 * @return string photoboard URL
 */
function block_culcourse_dashboard_get_photoboard_url($course, $roleid, $blockid) {
    global $COURSE, $CFG;
    $cname = explode(" ",$COURSE->fullname);
    $cid = $cname[0];
    $coursecontext = context_course::instance($COURSE->id);
    $url = new moodle_url(
        '/blocks/culcourse_dashboard/photoboard.php',
        array(
...
/**
 * block_culcourse_dashboard_get_coursecode_data()
 *
 ...
 */
function block_culcourse_dashboard_get_coursecode_data($coursecode) { //TODO: Candidate for a static utility method.
