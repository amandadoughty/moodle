...
$blockid      = optional_param('blockid', 0, PARAM_INT); // This are required.

$PAGE->set_url('/blocks/culcourse_dashboard/photoboard.php', array(
        'page' => $page,
        'perpage' => $perpage,
        'mode' => $mode,
...
$PAGE->add_body_class('path-block-culcourse_dashboard-photos');
...
if ($configdata = $DB->get_field('block_instances', 'configdata', array('id' => $blockid))) {
    ...
    if(isset($config->selectmoduleleaders)) {
            $moduleleaders = $config->selectmoduleleaders;
    }
}
...
$rolenamesurl = new moodle_url("$CFG->wwwroot/blocks/culcourse_dashboard/photoboard.php?contextid=$context->id&mode=$mode");
...
if (has_capability('block/culcourse_dashboard:viewallphotoboard', $context)) {
    ...
    if ($mycourses = enrol_get_my_courses()) {
        $courselist = array();
        $popupurl = new moodle_url('/blocks/culcourse_dashboard/photoboard.php?roleid='.$roleid.'&sifirst=&silast=');
...
if (count($rolenames) > 1) {
    ...
    echo '<label for="rolesform_jump">'.get_string('roles', 'block_culcourse_dashboard').'&nbsp;</label>';
        ...
        if ($rolematchcount > 0) {
            ...
            foreach ($userlist as $user) {
                ...
                if (in_array($user->id, $moduleleaders)) {
                    $moduleleaderstr = get_string('moduleleader', 'block_culcourse_dashboard');
                }
                ...
                if (!isset($hiddenfields['mobile'])) {
                    $row->cells[1]->text .= get_string('mobile', 'block_culcourse_dashboard') . get_string('labelsep', 'langconfig') . $user->phone2 . '<br/>';
                }
                ...
                if (!isset($hiddenfields['webapps'])) {
                    ...
                    if ((substr($user->username, 0, 1) == 'a') && (substr($user->idnumber, 0, 2) <> '88')) {
                        $links[] = html_writer::link(
                            ...
                            get_string('linktowebapps', 'block_culcourse_dashboard')
                            );
                    }
                }
                ...
                if (!isset($hiddenfields['forumposts'])) {
                    $links[] = html_writer::link(
                                ...
                                get_string('forum-posts', 'block_culcourse_dashboard')
                                );
                }
                ...
                if (!isset($hiddenfields['sendmessage'])) {
                    $links[] = html_writer::link(
                            ...
                            get_string('sendmessage', 'block_culcourse_dashboard')
                        );
                }
