...
class block_culcourse_listing_renderer extends plugin_renderer_base {
    ...
    protected function coursecat_courses(block_culcourse_listing_helper $chelper, $courses, $viewcourses, $totalcount = null) {
        ...
        foreach ($courses as $course) {
            ...
            if ($viewcourses || $enrolled) {
                ...
                $filtered = $filterlistfunction($course, $this->config, $years, $periods, $chelper->get_daterange_periods());
                $coursecount ++;

                if (!$filtered && !$firstfound) {
                    $firstfound = true;
                    $first = true;
                }
    ...
    public function coursecat_ajax($chelper) {
        ...
        if ($type === core_course_renderer::COURSECAT_TYPE_CATEGORY) {
            ...
            $data = \block_culcourse_listing\output\coursebox::coursecat_course_summary($chelper, $course);

            $content = $this->render_from_template('block_culcourse_listing/info', $data);
