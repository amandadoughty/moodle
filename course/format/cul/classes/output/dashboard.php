...
class dashboard implements templatable, renderable {
    ...
    public function quicklink_display($course) {
        ...
        if ($this->culconfig['showreadinglists'] == 2 || $this->userisediting) {
            $lnktxt = get_string('aspirelists', 'format_cul');
            $attrs  = array ();
            $liattrs = array();
            $class = '';
            $editurl = '';
            ...
            if ($this->userisediting) {
                list($editurl, $editicon, $editattrs) = format_cul_get_edit_link(
                    $course->id, 
                    'readinglists', 
            ...
            if (!$urldata) {
                ...
                $attrs['title'] = get_string('not-installed-readinglist', 'format_cul');
                $attrs['class'] = 'nolink';
                ...
                if (OK == $urldata['status']) {
                    ...
                    if ('module' == $listtype) {
                        $attrs['title'] = get_string('view-readinglist-module', 'format_cul');
                        $attrs['target'] = '_blank';
                    } else if ('module-year' == $listtype) {
                        $attrs['title'] = get_string('view-readinglist-module-year', 'format_cul');
                        $attrs['target'] = '_blank';
                    }
                } else if (NODATA == $urldata['status']) {
                    $attrs['title'] = get_string('no-readinglist', 'format_cul');
                    $attrs['class'] = 'nolink';
                    $url = 'javascript:void(0);';
                } else if (ERROR == $urldata['status']) {
                    $attrs['title'] = get_string('error-readinglist', 'format_cul');
                    $attrs['class'] = 'nolink';
                    $url = 'javascript:void(0);';
                }
            }
        ...
        if ($this->culconfig['showtimetable'] == 2 || $this->userisediting) {
            $lnktxt = get_string('timetable', 'format_cul');
            $attrs = array ();
            $liattrs = array();
            $class = '';
            $editurl = '';
            ...
            if (!$ttdata) {
                // Not installed or not configured.
                $attrs['title'] = get_string('not-installed-timetable', 'format_cul');
                $attrs['class'] = 'nolink';
                $url = 'javascript:void(0);';
            } else {
                if (OK == $ttdata['status']) {
                    $attrs['title']  = get_string('view-timetable', 'format_cul');
                    $attrs['target'] = '_blank';
                    $url = $ttdata['url'];
                } else if (NODATA == $ttdata['status']) {
                    $attrs['title']  = get_string('no-timetable', 'format_cul');
                    $attrs['class'] = 'nolink';
                    $attrs['target'] = '_blank';
                    $url = $ttdata['url'];
                } else if (ERROR == $ttdata['status']) {
                    $attrs['title'] = get_string('error-timetable', 'format_cul');
                    $attrs['class'] = 'nolink';
                    $url = 'javascript:void(0);';
                }
            }
        ...
        if ($this->culconfig['showgraderreport'] == 2 || $this->userisediting) {
            ...
            if (has_capability('gradereport/grader:view', $coursecontext)) { // Teacher, ...
                ...
                $attrs['title'] = get_string('view-graderreport', 'format_cul');
                ...
                $attrs['title'] = get_string('no-view-grades', 'format_cul');
                $attrs['class'] = 'nolink';
                $url = 'javascript:void(0);';
            }
        ...
        if ($this->culconfig['showcalendar'] == 2 || $this->userisediting) {
            ...
            $attrs['title'] = get_string('view-calendar', 'format_cul');
        ...
        if ($this->culconfig['showstudents'] == 2 || $this->userisediting) {
            ...
            if ($studentrole){
                ...
                if (count_role_users($studentrole->id, $coursecontext, false)){
                    $attrs['title']  = get_string('view-student-photoboard', 'format_cul', $alias);
                    $attrs['target'] = '';
                    $url = format_cul_get_photoboard_url($course, $studentrole->id);
                } else {
                    $attrs['class'] = 'nolink';
                    $attrs['title']  = get_string('no-view-student-photoboard', 'format_cul', $alias);
                    $url = 'javascript:void(0);';
                }
        ...
        if ($this->culconfig['showlecturers'] == 2 || $this->userisediting) {
            ...
            if ($lecturerrole){
                ...
                if (count_role_users($lecturerrole->id, $coursecontext, false)){
                    $attrs['title']  = get_string('view-lecturer-photoboard', 'format_cul', $alias);
                    $attrs['target'] = '';
                    $url = format_cul_get_photoboard_url($course, $lecturerrole->id);
                } else {
                    $attrs['class'] = 'nolink';
                    $attrs['title']  = get_string('no-view-lecturer-photoboard', 'format_cul', $alias);
                    $url = 'javascript:void(0);';
                }
        ...
        if ($this->culconfig['showcourseofficers'] == 2 || $this->userisediting) {
            ...
            if ($courseofficerrole){
                ...
                if (count_role_users($courseofficerrole->id, $coursecontext, false)){
                    $attrs['title']  = get_string('view-courseofficer-photoboard', 'format_cul', $alias);
                    $attrs['target'] = '';
                    $url = format_cul_get_photoboard_url($course, $courseofficerrole->id);
                } else {
                    $attrs['class'] = 'nolink';
                    $attrs['title']  = get_string('no-view-courseofficer-photoboard', 'format_cul', $alias);
                    $url = 'javascript:void(0);';
                }
        ...
        if ($this->culconfig['showmedia'] == 2 || $this->userisediting) {
            $lnktxt = get_string('media', 'format_cul');
            $attrs  = array ();
            $liattrs = array();
            $class = '';
            $editurl = '';
            ...
            $attrs['title'] = get_string('view-media', 'format_cul');
