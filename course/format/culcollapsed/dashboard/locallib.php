...
/**
 * format_culcollapsed_get_reading_list_url_data()
 *
 * @param mixed $course
 * @return
 */
function format_culcollapsed_get_reading_list_url_data($course) {
    global $CFG;

    $site = trim(get_config('aspirelists', 'targetAspire')); // 1.x: $CFG->block_aspirelists_targetAspire;
    $codedata = format_culcollapsed_get_coursecode_data($course);
    ...
    $data = format_culcollapsed_get_reading_list_data($path, $url, $connectiontimeout, $transfertimeout);
    ...
    if (OK == $data['status']) {
        ...
        $data = format_culcollapsed_get_reading_list_data($path, "{$path}.json", $connectiontimeout, $transfertimeout);
        if (OK == $data['status']) {
            return array('status' => OK, 'listtype' => 'module', 'url' => $path);
        }
    }
...
/**
 * format_culcollapsed_get_reading_list_data()
 *
 ...
 */
function format_culcollapsed_get_reading_list_data($path, $url, $connectiontimeout = null, $transfertimeout = null) {
...
/**
 * format_culcollapsed_get_timetable_url()
 *
 * @param mixed $course
 * @return string timetable URL
 */
function format_culcollapsed_get_timetable_url($course) {
    global $COURSE, $CFG;

    $ttdata = array('url' => null, 'status' => 1, 'httpcode' => null, 'error' => null);
    $codedata = format_culcollapsed_get_coursecode_data($COURSE->shortname);
...
/**
 * format_culcollapsed_get_photoboard_url()
 *
 * @param mixed $course
 * @return string photoboard URL
 */
function format_culcollapsed_get_photoboard_url($course, $roleid) {
    global $COURSE, $CFG;
...
/**
 * format_culcollapsed_get_coursecode_data()
 *
 ...
 */
function format_culcollapsed_get_coursecode_data($coursecode) { //TODO: Candidate for a static utility method.
...
function format_culcollapsed_quicklink_visibility($courseid, $name, $value) {
    global $DB;
...
function format_culcollapsed_get_edit_link($courseid, $name, $value) {    
    ...
    // $name = 'show' . $name;
    $editattrs['title'] = get_string($title, 'format_culcollapsed');
    $editattrs['class'] = 'quicklinkedit';
    $editurl = new moodle_url(
        '/course/format/culcollapsed/dashboard/quicklink_edit.php',
        array(
            'courseid' => $courseid, 
            'name' => $name,
            'value' => $newvalue,
...
function format_culcollapsed_get_lti_instances($course, $type) {
    global $CFG, $USER;
