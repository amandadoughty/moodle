...
namespace format_culcollapsed\output\dashboard;
...
class renderer extends \plugin_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->config = get_config('format_culcollapsed');
        ...
        /* Since format_culcollapsed_renderer::section_edit_controls() only displays the 'Set current section' control when editing
        ...
        if ($this->userisediting) {
            ...
            $page->requires->js_call_amd('format_culcollapsed/quicklinks', 'initialize', array($arguments));
        }
    }
    ...
    /**
     * format_culcollapsed_renderer::quicklink_display()
     *
    ...
    public function quicklink_display($course) {
        ...
        if ($this->tcsettings['showreadinglists'] == 2 || $this->userisediting) {
            ...
            if ($this->userisediting) {
                $editlink = format_culcollapsed_get_edit_link(
                    $course->id, 
                    'readinglists', 
            ...
            if (!$urldata) {
                ...
                $rllnktxt = get_string('aspirelists', 'format_culcollapsed');
                $rlattrs['title'] = get_string('not-installed-readinglist', 'format_culcollapsed');
                ...
                if (OK == $urldata['status']) {
                    ...
                    if ('module' == $listtype) {
                        $rlattrs['title'] = get_string('view-readinglist-module', 'format_culcollapsed');
                        $rlattrs['target'] = '_blank';
                    } else if ('module-year' == $listtype) {
                        $rlattrs['title'] = get_string('view-readinglist-module-year', 'format_culcollapsed');
                        $rlattrs['target'] = '_blank';
                    }
                } else if (NODATA == $urldata['status']) {
                    $rlattrs['title'] = get_string('no-readinglist', 'format_culcollapsed');
                    $rlattrs['class'] = 'nolink';
                    $rlurl = 'javascript:void(0);';
                } else if (ERROR == $urldata['status']) {
                    $rlattrs['title'] = get_string('error-readinglist', 'format_culcollapsed');
                    $rlattrs['class'] = 'nolink';
                    $rlurl = 'javascript:void(0);';
                }
            }
        ...
        if (($this->tcsettings['showecho'] == 2) || $this->userisediting) {
            ...
            $urldata = format_culcollapsed_get_echo_data($course);
            ...
            if (!$urldata) {
                // Not installed or not configured.
                $elnktxt = get_string('echocenter', 'format_culcollapsed');
                $eattrs['title'] = get_string('not-installed-echo', 'format_culcollapsed');
                $eattrs['class'] = 'nolink';
                ...
                if (OK == $urldata['status']) {
                    $eattrs['title'] = get_string('view-echo-module', 'format_culcollapsed');
                    $eattrs['target'] = '_blank';
                    $eurl = $urldata['url'];
                } else if (NODATA == $urldata['status']) {
                    $eattrs['title'] = get_string('no-echo', 'format_culcollapsed');
                    $eattrs['class'] = 'nolink';
                    $eurl = 'javascript:void(0);';
                } else if (ERROR == $urldata['status']) {
                    $eattrs['title'] = get_string('error-echo', 'format_culcollapsed');
                    $eattrs['class'] = 'nolink';
                    $eurl = 'javascript:void(0);';
                }
            }
        ...
        if ($this->tcsettings['showtimetable'] == 2 || $this->userisediting) {
            ...
            if (!$ttdata) {
                // Not installed or not configured.
                $ttattrs['title'] = get_string('not-installed-timetable', 'format_culcollapsed');
                $ttattrs['class'] = 'nolink';
                $tturl = 'javascript:void(0);';
            } else {
                if (OK == $ttdata['status']) {
                    $ttattrs['title']  = get_string('view-timetable', 'format_culcollapsed');
                    $ttattrs['target'] = '_blank';
                    $tturl = $ttdata['url'];
                } else if (NODATA == $ttdata['status']) {
                    $ttattrs['title']  = get_string('no-timetable', 'format_culcollapsed');
                    $ttattrs['class'] = 'nolink';
                    $ttattrs['target'] = '_blank';
                    $tturl = $ttdata['url'];
                } else if (ERROR == $ttdata['status']) {
                    $ttattrs['title'] = get_string('error-timetable', 'format_culcollapsed');
                    $ttattrs['class'] = 'nolink';
                    $tturl = 'javascript:void(0);';
                }
            }
            ...
            $ttlnktx = get_string('timetable', 'format_culcollapsed');
        ...
        if ($this->tcsettings['showgraderreport'] == 2 || $this->userisediting) {
            ...
            if (has_capability('gradereport/grader:view', $coursecontext)) { // Teacher, ...
                ...
                $gattrs['title'] = get_string('view-graderreport', 'format_culcollapsed');
                ...
                $gattrs['title'] = get_string('no-view-grades', 'format_culcollapsed');
                $gattrs['class'] = 'nolink';
        ...
        if ($this->tcsettings['showcalendar'] == 2 || $this->userisediting) {
            ...
            $cattrs['title'] = get_string('view-calendar', 'format_culcollapsed');
        ...
        if ($this->tcsettings['showstudents'] == 2 || $this->userisediting) {
            ...
            if ($studentrole){
                ...
                if (count_role_users($studentrole->id, $coursecontext, false)){
                    $phattrs['title']  = get_string('view-student-photoboard', 'format_culcollapsed', $alias);
                    $phattrs['target'] = '';
                    $phurl = format_culcollapsed_get_photoboard_url($course, $studentrole->id);
                } else {
                    $phattrs['class'] = 'nolink';
                    $phattrs['title']  = get_string('no-view-student-photoboard', 'format_culcollapsed', $alias);
        ...
        if ($this->tcsettings['showlecturers'] == 2 || $this->userisediting) {
            ...
            if ($lecturerrole){
                ...
                if (count_role_users($lecturerrole->id, $coursecontext, false)){
                    $phattrs['title']  = get_string('view-lecturer-photoboard', 'format_culcollapsed', $alias);
                    $phattrs['target'] = '';
                    $phurl = format_culcollapsed_get_photoboard_url($course, $lecturerrole->id);
                } else {
                    $phattrs['class'] = 'nolink';
                    $phattrs['title']  = get_string('no-view-lecturer-photoboard', 'format_culcollapsed', $alias);
                    $phurl = 'javascript:void(0);';
                }
        ...
        if ($this->tcsettings['showcourseofficers'] == 2 || $this->userisediting) {
            ...
            if ($courseofficerrole){
                ...
                if (count_role_users($courseofficerrole->id, $coursecontext, false)){
                    $phattrs['title']  = get_string('view-courseofficer-photoboard', 'format_culcollapsed', $alias);
                    $phattrs['target'] = '';
                    $phurl = format_culcollapsed_get_photoboard_url($course, $courseofficerrole->id);
                } else {
                    $phattrs['class'] = 'nolink';
                    $phattrs['title']  = get_string('no-view-courseofficer-photoboard', 'format_culcollapsed', $alias);
                    $phurl = 'javascript:void(0);';
                }
        ...
        if ($this->tcsettings['showmedia'] == 2 || $this->userisediting) {
            ...
            $klnktxt = get_string('media', 'format_culcollapsed');

            if ($this->userisediting&& ($this->tcsettings['showmedia'] != 2)) {
                    $class = 'linkhidden';                
            }

            $kattrs['title'] = get_string('view-media', 'format_culcollapsed');
        ...
        $title = get_string('quicklinks', 'format_culcollapsed');
    ...
    /**
     * format_culcollapsed_renderer::activity_modules_display()
     *
     * @param stdClass $course
     * @return
     */
    public function activity_modules_display($course) {
        ...
        $title = get_string('activities', 'format_culcollapsed');
        $panelheader = html_writer::tag('div', $title, array('class'=>'panel-header ui-corner-all'));
        $navcontent  = html_writer::tag('nav', $content, array('class'=>'linkscontainer'));
        $output .= html_writer::tag('div', $panelheader . $navcontent, array('class'=>'dash-panel clearfix'));
    ...
    /**
     * format_culcollapsed_renderer::exttools_modules_display()
     *
     * @param stdClass $course
