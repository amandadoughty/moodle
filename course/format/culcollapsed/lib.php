...
require_once($CFG->dirroot . '/course/format/culcollapsed/dashboard/lib.php');

class format_culcollapsed extends format_base {
    ...
    public function get_section_name($section) {
        ...
        return $this->get_culcollapsed_section_name($course, $section, false);
    }
     ...
     * @return string The section name.
     */
    public function get_culcollapsed_section_name($course, $section, $additional) {
        ...
        if ((string) $thesection->name !== '') {
            ...
            $o = get_string('section0name', 'format_culcollapsed');
        } else {
            if (($tcsettings['layoutstructure'] == 1) || ($tcsettings['layoutstructure'] == 4)) {
                $o = get_string('sectionname', 'format_culcollapsed') . ' ' . $thesection->section;
            } else {
                $o .= $this->get_section_dates($section, $course, $tcsettings);
            }
        }
        ...
        if (($additional == true) && ($thesection->section != 0)) {
            switch ($tcsettings['layoutelement']) {
                ...
                case 4:
                    // The word 'Toggle'.
                    $o .= '<div class="cttoggle"> - ' .get_string('culcollapsedtoggle', 'format_culcollapsed') . '</div>';
                    break;
            }
        }
    ...
    public function get_section_dates($section, $course, $tcsettings) {
        ...
        if ($tcsettings['layoutstructure'] == 5) {
            $day = $this->format_culcollapsed_get_section_day($section, $course);

            $weekday = userdate($day, $dateformat);
            $o = $weekday;
        } else {
            $dates = $this->format_culcollapsed_get_section_dates($section, $course);
    ...
    public function ajax_section_move() {
        ...
        if ($sections = $modinfo->get_section_info_all()) {
            foreach ($sections as $number => $section) {
                $titles[$number] = $this->get_culcollapsed_section_name($course, $section, true);
    ...
    public function section_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($sectionformatoptions['donotshowdate']['label'])) {
            $sectionformatoptionsedit = array(
                'donotshowdate' => array(
                    'label' => new lang_string('donotshowdate', 'format_culcollapsed'),
                    'help' => 'donotshowdate',
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            $defaulttgfgcolour = get_config('format_culcollapsed', 'defaulttgfgcolour');
            if ($defaulttgfgcolour[0] == '#') {
                $defaulttgfgcolour = substr($defaulttgfgcolour, 1);
            }
            $defaulttgfghvrcolour = get_config('format_culcollapsed', 'defaulttgfghvrcolour');
            if ($defaulttgfghvrcolour[0] == '#') {
                $defaulttgfghvrcolour = substr($defaulttgfghvrcolour, 1);
            }
            $defaulttgbgcolour = get_config('format_culcollapsed', 'defaulttgbgcolour');
            if ($defaulttgbgcolour[0] == '#') {
                $defaulttgbgcolour = substr($defaulttgbgcolour, 1);
            }
            $defaulttgbghvrcolour = get_config('format_culcollapsed', 'defaulttgbghvrcolour');
            ...
            $courseformatoptions = array(
                ...
                'showsectionsummary' => array(
                    'default' => get_config('format_culcollapsed', 'defaultshowsectionsummary'),
                    'type' => PARAM_INT,
                ),
                'showcoursesummary' => array(
                    'default' => get_config('format_culcollapsed', 'defaultshowcoursesummary'),
                    'type' => PARAM_INT,
                ),

                // Dashboard settings will be spliced here.

                'displayinstructions' => array(
                    'default' => get_config('format_culcollapsed', 'defaultdisplayinstructions'),
                    'type' => PARAM_INT,
                ),
                'layoutelement' => array(
                    'default' => get_config('format_culcollapsed', 'defaultlayoutelement'),
                    'type' => PARAM_INT,
                ),
                'layoutstructure' => array(
                    'default' => get_config('format_culcollapsed', 'defaultlayoutstructure'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumns' => array(
                    'default' => get_config('format_culcollapsed', 'defaultlayoutcolumns'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumnorientation' => array(
                    'default' => get_config('format_culcollapsed', 'defaultlayoutcolumnorientation'),
                    'type' => PARAM_INT,
                ),
                'togglealignment' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttogglealignment'),
                    'type' => PARAM_INT,
                ),
                'toggleiconposition' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttoggleiconposition'),
                    'type' => PARAM_INT,
                ),
                'toggleiconset' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttoggleiconset'),
                    'type' => PARAM_ALPHA,
                ),
                'onesection' => array(
                    'default' => get_config('format_culcollapsed', 'defaultonesection'),
                    'type' => PARAM_INT,
                ),
                'toggleallhover' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttoggleallhover'),
                    'type' => PARAM_INT,
                ),
                ...
                'toggleforegroundopacity' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttgfgopacity'),
                    'type' => PARAM_RAW,
                ),
                ...
                'toggleforegroundhoveropacity' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttgbghvropacity'),
                    'type' => PARAM_RAW,
                ),
                ...
                'togglebackgroundopacity' => array(
                    'default' => get_config('format_culcollapsed', 'defaulttgbgopacity'),
                    'type' => PARAM_RAW,
                ),
        ...
        $dashboard = new format_culcollapsed_dashboard();        
        ...
        if ($foreditform && !isset($courseformatoptions['displayinstructions']['label'])) {
            ...
            $courseformatoptionsedit = array(
                'numsections' => array(
                    'label' => new lang_string('numbersections', 'format_culcollapsed'),
                    'element_type' => 'select',
                ...
                'displayinstructions' => array(
                    'label' => new lang_string('displayinstructions', 'format_culcollapsed'),
                ...
                'showsectionsummary' => array(
                    'label' => new lang_string('setshowsectionsummary', 'format_culcollapsed'),
                ...
                'showcoursesummary' => array(
                    'label' => new lang_string('setshowcoursesummary', 'format_culcollapsed'),
            ...
            if (has_capability('format/culcollapsed:changelayout', $context)) {
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => new lang_string('setlayoutelements', 'format_culcollapsed'),
                    ...
                    'element_attributes' => array( // In insertion order and not numeric for sorting purposes.
                        array(
                            ...
                            1 => new lang_string('setlayout_all', 'format_culcollapsed'),
                            // Toggle word and toggle section x.
                            3 => new lang_string('setlayout_toggle_word_section_x', 'format_culcollapsed'),
                            // Toggle word and section number.
                            2 => new lang_string('setlayout_toggle_word_section_number', 'format_culcollapsed'),
                            // Toggle section x and section number.
                            5 => new lang_string('setlayout_toggle_section_x_section_number', 'format_culcollapsed'),
                            // Toggle word.
                            4 => new lang_string('setlayout_toggle_word', 'format_culcollapsed'),
                            // Toggle section x.
                            8 => new lang_string('setlayout_toggle_section_x', 'format_culcollapsed'),
                            // Section number.
                            6 => new lang_string('setlayout_section_number', 'format_culcollapsed'),
                            // No additions.
                            7 => new lang_string('setlayout_no_additions', 'format_culcollapsed'))
                    )
                );
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => new lang_string('setlayoutstructure', 'format_culcollapsed'),
                    ...
                    'element_attributes' => array(
                        array(
                            // Topic.
                            1 => new lang_string('setlayoutstructuretopic', 'format_culcollapsed'),
                            // Week.
                            2 => new lang_string('setlayoutstructureweek', 'format_culcollapsed'),
                            // Current Week First.
                            3 => new lang_string('setlayoutstructurelatweekfirst', 'format_culcollapsed'),
                            // Current Topic First.
                            4 => new lang_string('setlayoutstructurecurrenttopicfirst', 'format_culcollapsed'),
                            // Day.
                            5 => new lang_string('setlayoutstructureday', 'format_culcollapsed'))
                    )
                );
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => new lang_string('setlayoutcolumns', 'format_culcollapsed'),
                    'help' => 'setlayoutcolumns',
                    'help_component' => 'format_culcollapsed',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('one', 'format_culcollapsed'),   // Default.
                ...
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => new lang_string('setlayoutcolumnorientation', 'format_culcollapsed'),
                    'help' => 'setlayoutcolumnorientation',
                    'help_component' => 'format_culcollapsed',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('columnvertical', 'format_culcollapsed'),
                              2 => new lang_string('columnhorizontal', 'format_culcollapsed')) // Default.
                    )
                );
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => new lang_string('settoggleiconposition', 'format_culcollapsed'),
                    'help' => 'settoggleiconposition',
                    'help_component' => 'format_culcollapsed',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('left', 'format_culcollapsed'),   // Left.
                              2 => new lang_string('right', 'format_culcollapsed'))  // Right.
                    )
                );
                $courseformatoptionsedit['onesection'] = array(
                    'label' => new lang_string('onesection', 'format_culcollapsed'),
                    'help' => 'onesection',
                    'help_component' => 'format_culcollapsed',
                    'element_type' => 'select',
                    'element_attributes' => array(
                ...
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => get_config('format_culcollapsed', 'defaultlayoutelement'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => get_config('format_culcollapsed', 'defaultlayoutstructure'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => get_config('format_culcollapsed', 'defaultlayoutcolumns'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => get_config('format_culcollapsed', 'defaultlayoutcolumnorientation'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttoggleiconposition'), 'element_type' => 'hidden');
                $courseformatoptionsedit['onesection'] = array(
                    'label' => get_config('format_culcollapsed', 'defaultonesection'), 'element_type' => 'hidden');
            }

            if (has_capability('format/culcollapsed:changetogglealignment', $context)) {
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => new lang_string('settogglealignment', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttogglealignment'), 'element_type' => 'hidden');
            }

            if (has_capability('format/culcollapsed:changetoggleiconset', $context)) {
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => new lang_string('settoggleiconset', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => new lang_string('settoggleallhover', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttoggleiconset'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttoggleallhover'), 'element_type' => 'hidden');
            }

            if (has_capability('format/culcollapsed:changecolour', $context)) {
                $opacityvalues = array(
                    '0.0' => '0.0',
                    '0.1' => '0.1',
                    '0.2' => '0.2',
                ...
                $courseformatoptionsedit['toggleforegroundcolour'] = array(
                    'label' => new lang_string('settoggleforegroundcolour', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleforegroundopacity'] = array(
                    'label' => new lang_string('settoggleforegroundopacity', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => new lang_string('settoggleforegroundhovercolour', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleforegroundhoveropacity'] = array(
                    'label' => new lang_string('settoggleforegroundhoveropacity', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => new lang_string('settogglebackgroundcolour', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['togglebackgroundopacity'] = array(
                    'label' => new lang_string('settogglebackgroundopacity', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => new lang_string('settogglebackgroundhovercolour', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['togglebackgroundhoveropacity'] = array(
                    'label' => new lang_string('settogglebackgroundhoveropacity', 'format_culcollapsed'),
                ...
                $courseformatoptionsedit['toggleforegroundopacity'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttgfgopacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => $defaulttgfghvrcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleforegroundhoveopacity'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttgfghvropacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => $defaulttgbgcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundopacity'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttgbgopacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => $defaulttgbghvrcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundhoveopacity'] = array(
                    'label' => get_config('format_culcollapsed', 'defaulttgbghvropacity'), 'element_type' => 'hidden');
            }
            $readme = new moodle_url('/course/format/culcollapsed/Readme.md');
            $readme = html_writer::link($readme, 'Readme.md', array('target' => '_blank'));
            $courseformatoptionsedit['readme'] = array(
                    'label' => get_string('readme_title', 'format_culcollapsed'),
                    'element_type' => 'static',
                    'element_attributes' => array(get_string('readme_desc', 'format_culcollapsed', array('url' => $readme)))
                );
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        global $CFG, $OUTPUT;
        MoodleQuickForm::registerElementType('tccolourpopup', "$CFG->dirroot/course/format/culcollapsed/js/tc_colourpopup.php",
        ...
        if ($forsection == false) {
            ...
            $selectmoduleleaderhdr = $mform->addElement('header', 'selectmoduleleadershdr', get_string('setselectmoduleleadershdr', 'format_culcollapsed'));
            $mform->addHelpButton('selectmoduleleadershdr', 'setselectmoduleleadershdr', 'format_culcollapsed', '', true);
            array_splice($elements, -16, 0, [$selectmoduleleaderhdr]);

            // Put dashboard settings in own dropdown.
            $dashboardhdr = $mform->addElement('header', 'dashboardhdr', get_string('setdashboardhdr', 'format_culcollapsed'));
            array_splice($elements, 4, 0, [$dashboardhdr]);
            
            // Put settings that extra permissions are required for in own dropdown.
            $advancedhdr = $mform->addElement('header', 'advancedhdr', get_string('setadvancedhdr', 'format_culcollapsed'));
            array_splice($elements, -15, 0, [$advancedhdr]);

            $context = $this->get_context();

            $changelayout = has_capability('format/culcollapsed:changelayout', $context);
            $changecolour = has_capability('format/culcollapsed:changecolour', $context);
            $changetogglealignment = has_capability('format/culcollapsed:changetogglealignment', $context);
            $changetoggleiconset = has_capability('format/culcollapsed:changetoggleiconset', $context);
            $resetall = is_siteadmin($USER); // Site admins only.

            $elements[] = $mform->addElement('header', 'ctreset', get_string('ctreset', 'format_culcollapsed'));
            $mform->addHelpButton('ctreset', 'ctreset', 'format_culcollapsed', '', true);

            $resetelements = array();
            $checkboxname = get_string('resetdisplayinstructions', 'format_culcollapsed').
                $OUTPUT->help_icon('resetdisplayinstructions', 'format_culcollapsed');
            $resetelements[] =& $mform->createElement('checkbox', 'resetdisplayinstructions', '', $checkboxname);

            if ($changelayout) {
                $checkboxname = get_string('resetlayout', 'format_culcollapsed').$OUTPUT->help_icon('resetlayout', 'format_culcollapsed');
                $resetelements[] =& $mform->createElement('checkbox', 'resetlayout', '', $checkboxname);
            }

            if ($changecolour) {
                $checkboxname = get_string('resetcolour', 'format_culcollapsed').$OUTPUT->help_icon('resetcolour', 'format_culcollapsed');
                $resetelements[] =& $mform->createElement('checkbox', 'resetcolour', '', $checkboxname);
            }

            if ($changetogglealignment) {
                $checkboxname = get_string('resettogglealignment', 'format_culcollapsed').
                    $OUTPUT->help_icon('resettogglealignment', 'format_culcollapsed');
                $resetelements[] =& $mform->createElement('checkbox', 'resettogglealignment', '', $checkboxname);
            }

            if ($changetoggleiconset) {
                $checkboxname = get_string('resettoggleiconset', 'format_culcollapsed').
                    $OUTPUT->help_icon('resettoggleiconset', 'format_culcollapsed');
                $resetelements[] =& $mform->createElement('checkbox', 'resettoggleiconset', '', $checkboxname);
            }
            $elements[] = $mform->addGroup($resetelements, 'resetgroup', get_string('resetgrp', 'format_culcollapsed'), null, false);

            if ($resetall) {
                $resetallelements = array();

                $checkboxname = get_string('resetalldisplayinstructions', 'format_culcollapsed').
                    $OUTPUT->help_icon('resetalldisplayinstructions', 'format_culcollapsed');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalldisplayinstructions', '', $checkboxname);

                $checkboxname = get_string('resetalllayout', 'format_culcollapsed').
                    $OUTPUT->help_icon('resetalllayout', 'format_culcollapsed');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalllayout', '', $checkboxname);

                $checkboxname = get_string('resetallcolour', 'format_culcollapsed').
                    $OUTPUT->help_icon('resetallcolour', 'format_culcollapsed');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetallcolour', '', $checkboxname);

                $checkboxname = get_string('resetalltogglealignment', 'format_culcollapsed').
                    $OUTPUT->help_icon('resetalltogglealignment', 'format_culcollapsed');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalltogglealignment', '', $checkboxname);

                $checkboxname = get_string('resetalltoggleiconset', 'format_culcollapsed').
                    $OUTPUT->help_icon('resetalltoggleiconset', 'format_culcollapsed');
                ...
                $elements[] = $mform->addGroup($resetallelements, 'resetallgroup',
                    get_string('resetallgrp', 'format_culcollapsed'), null, false);
            }
        }
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if ($this->validate_colour($data['toggleforegroundcolour']) === false) {
            $retr['toggleforegroundcolour'] = get_string('colourrule', 'format_culcollapsed');
        }
        if ($this->validate_colour($data['toggleforegroundhovercolour']) === false) {
            $retr['toggleforegroundhovercolour'] = get_string('colourrule', 'format_culcollapsed');
        }
        if ($this->validate_colour($data['togglebackgroundcolour']) === false) {
            $retr['togglebackgroundcolour'] = get_string('colourrule', 'format_culcollapsed');
        }
        if ($this->validate_colour($data['togglebackgroundhovercolour']) === false) {
            $retr['togglebackgroundhovercolour'] = get_string('colourrule', 'format_culcollapsed');
        }
    ...
    public function update_course_format_options($data, $oldcourse = null) {
        ...
        if (($resetalldisplayinstructions) ||
            ...
            $this->reset_culcollapsed_setting(0, $resetalldisplayinstructions, $resetalllayout, $resetallcolour,
            ...
            ($resettoggleiconset)) {
            $this->reset_culcollapsed_setting($this->courseid, $resetdisplayinstructions, $resetlayout, $resetcolour,
    ...
    public function is_section_current($section) {
        ...
        if (($tcsettings['layoutstructure'] == 2) || ($tcsettings['layoutstructure'] == 3)) {
            ...
            $timenow = time();
            $dates = $this->format_culcollapsed_get_section_dates($section, $this->get_course());
            ...
            $timenow = time();
            $day = $this->format_culcollapsed_get_section_day($section, $this->get_course());
            $onedayseconds = 86400;
     ...
     */
    private function format_culcollapsed_get_section_dates($section, $course) {
     ...
     */
    private function format_culcollapsed_get_section_day($section, $course) {
        $onedayseconds = 86400;
        /* Hack alert. We add 2 hours to avoid possible DST problems. (e.g. we go into daylight
           savings and the date changes. */
        $startdate = $course->startdate + 7200;
     ...
     */
    public function reset_culcollapsed_setting($courseid, $displayinstructions, $layout, $colour, $togglealignment, $toggleiconset) {
        global $DB, $USER;
        ...
        if ($displayinstructions && $resetallifall) {
            $updatedata['displayinstructions'] = get_config('format_culcollapsed', 'defaultdisplayinstructions');
            $updatedisplayinstructions = true;
        }
        if ($layout && has_capability('format/culcollapsed:changelayout', $context) && $resetallifall) {
            $updatedata['layoutelement'] = get_config('format_culcollapsed', 'defaultlayoutelement');
            $updatedata['layoutstructure'] = get_config('format_culcollapsed', 'defaultlayoutstructure');
            $updatedata['layoutcolumns'] = get_config('format_culcollapsed', 'defaultlayoutcolumns');
            $updatedata['layoutcolumnorientation'] = get_config('format_culcollapsed', 'defaultlayoutcolumnorientation');
            $updatedata['toggleiconposition'] = get_config('format_culcollapsed', 'defaulttoggleiconposition');
            $updatedata['onesection'] = get_config('format_culcollapsed', 'defaultonesection');
            $updatedata['showsectionsummary'] = get_config('format_culcollapsed', 'defaultshowsectionsummary');
            $updatelayout = true;
        }
        if ($togglealignment && has_capability('format/culcollapsed:changetogglealignment', $context) && $resetallifall) {
            $updatedata['togglealignment'] = get_config('format_culcollapsed', 'defaulttogglealignment');
            $updatetogglealignment = true;
        }
        if ($colour && has_capability('format/culcollapsed:changecolour', $context) && $resetallifall) {
            $updatedata['toggleforegroundcolour'] = get_config('format_culcollapsed', 'defaulttgfgcolour');
            $updatedata['toggleforegroundopacity'] = get_config('format_culcollapsed', 'defaulttgfgopacity');
            $updatedata['toggleforegroundhovercolour'] = get_config('format_culcollapsed', 'defaulttgfghvrcolour');
            $updatedata['toggleforegroundhoveropacity'] = get_config('format_culcollapsed', 'defaulttgfghvropacity');
            $updatedata['togglebackgroundcolour'] = get_config('format_culcollapsed', 'defaulttgbgcolour');
            $updatedata['togglebackgroundopacity'] = get_config('format_culcollapsed', 'defaulttgbgopacity');
            $updatedata['togglebackgroundhovercolour'] = get_config('format_culcollapsed', 'defaulttgbghvrcolour');
            $updatedata['togglebackgroundhoveropacity'] = get_config('format_culcollapsed', 'defaulttgbghvropacity');
            $updatecolour = true;
        }
        if ($toggleiconset && has_capability('format/culcollapsed:changetoggleiconset', $context) && $resetallifall) {
            $updatedata['toggleiconset'] = get_config('format_culcollapsed', 'defaulttoggleiconset');
            $updatedata['toggleallhover'] = get_config('format_culcollapsed', 'defaulttoggleallhover');
     ...
     * in the old 'format_culcollapsed_settings' table.
     * @param int $courseid If not 0, then a specific course to reset.
     ...
     */
    public function restore_culcollapsed_setting($courseid, $layoutelement, $layoutstructure, $layoutcolumns, $tgfgcolour,
        ...
        $lco = get_config('format_culcollapsed', 'defaultlayoutcolumnorientation');
        if (empty($lco)) {
            ...
            $data['displayinstructions'] = get_config('format_culcollapsed', 'defaultdisplayinstructions');
            $data['layoutcolumnorientation'] = get_config('format_culcollapsed', 'defaultlayoutcolumnorientation');
            $data['showsectionsummary'] = get_config('format_culcollapsed', 'defaultshowsectionsummary');
            $data['togglealignment'] = get_config('format_culcollapsed', 'defaulttogglealignment');
            $data['toggleallhover'] = get_config('format_culcollapsed', 'defaulttoggleallhover');
            $data['toggleiconposition'] = get_config('format_culcollapsed', 'defaulttoggleiconposition');
            $data['toggleiconset'] = get_config('format_culcollapsed', 'defaulttoggleiconset');
        }
     ...
     */
    public function update_culcollapsed_columns_setting($layoutcolumns) {
        // Create data array.
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_culcollapsed');
        }
        if (empty($editlabel)) {
            $course = $this->get_course();
            $title = $this->get_culcollapsed_section_name($course, $section, false);
            $editlabel = new lang_string('newsectionname', 'format_culcollapsed', $title);
        }
 ...
 * @return \core\output\inplace_editable
 */
function format_culcollapsed_inplace_editable($itemtype, $itemid, $newvalue) {
    global $CFG;
    ...
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        ...
        $section = $DB->get_record_sql(
            ...
            array($itemid, 'culcollapsed'), MUST_EXIST);
 ...
 */
function callback_culcollapsed_definition() {
    return get_string('sectionname', 'format_culcollapsed');
}
