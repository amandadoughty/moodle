...
use format_culcollapsed\output\dashboard;
...
class format_culcollapsed_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->togglelib = new culcollapsed_togglelib;
        $this->courseformat = course_get_format($page->course); // Needed for collapsed topics settings retrieval.

        /* Since format_culcollapsed_renderer::section_edit_control_items() only displays the 'Set current section' control when editing
    ...
    protected function start_section_list() {
        ...
        $o = '';
        $dashboard = new dashboard($COURSE, $this->tcsettings);
    ...
    protected function section_edit_control_items($course, $section, $onsectionpage = false) {
        ...
        if ((($this->tcsettings['layoutstructure'] == 1) || ($this->tcsettings['layoutstructure'] == 4)) &&
            ...
            if ($course->marker == $section->section) {  // Show the "light globe" on/off.
                $url->param('marker', 0);
                $markedthissection = get_string('markedthissection', 'format_culcollapsed');
        ...
        if ($isstealth) {
            $url = $url = new moodle_url(
                '/course/format/culcollapsed/reinstatesection.php', 
                array(
                    'id' => $section->section, 
                    'courseid' => $course->id,
                    'sesskey' => sesskey()
            ...
            $reinstate = get_string('reinstate', 'format_culcollapsed');
            $reinstatethissection = get_string('reinstatethissection', 'format_culcollapsed');
...
// var_dump($item);
    ...
    protected function section_summary($section, $course, $mods) {
        ...
        $o = '';
        $title = $this->courseformat->get_culcollapsed_section_name($course, $section, false);
        $liattributes = array(
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        $this->page->requires->js_init_call('M.format_culcollapsed.init', array(
            $course->id,
            $toggles,
            $course->numsections,
        ...
        set_user_preference('culcollapsed_toggle_'.$course->id, $toggles);
    }
    ...
    protected function display_instructions() {
        ...
        $o .= html_writer::tag('p', get_string('instructions', 'format_culcollapsed'),
            array('class' => 'culcollapsed-display-instructions')
        );
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('li');
   ...
   /**
    ...
    protected function section_insertsection($course, $id) {
        $o = html_writer::start_tag('div', array('class' => 'insertsection'));

        // Increase number of sections.
        $straddsection = get_string('addsection', 'format_culcollapsed');
        $url = new moodle_url('/course/format/culcollapsed/insertsection.php',
        ...
        $o .= html_writer::end_tag('div'); // End .insertsection.
