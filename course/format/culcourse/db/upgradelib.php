...
/**
 * Upgrade scripts for course format "CUL Course"
 *
...
/**
 * This method finds all courses in "culcourse" format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@link format_culcourse_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 */
function format_culcourse_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => "culcourse", 'format2' => "culcourse", 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_culcourse_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => "culcourse", 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_culcourse_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
