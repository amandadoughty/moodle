...
if (count($rolenames) > 1) {
    ...
    echo '<label for="rolesform_jump">'.get_string('roles', 'format_culcourse').'&nbsp;</label>';
        ...
        if ($rolematchcount > 0) {
            ...
            foreach ($userlist as $user) {
                ...
                if (in_array($user->id, $moduleleaders)) {
                    $moduleleaderstr = get_string('moduleleader', 'format_culcourse');
                }
                ...
                if (!isset($hiddenfields['mobile'])) {
                    $row->cells[1]->text .= get_string('mobile', 'format_culcourse') . get_string('labelsep', 'langconfig') . $user->phone2 . '<br/>';
                }
                ...
                if (!isset($hiddenfields['webapps'])) {
                    ...
                    if ((substr($user->username, 0, 1) == 'a') && (substr($user->idnumber, 0, 2) <> '88')) {
                        $links[] = html_writer::link(
                            ...
                            get_string('linktowebapps', 'format_culcourse')
                            );
                    }
                }
                ...
                if (!isset($hiddenfields['forumposts'])) {
                    $links[] = html_writer::link(
                                ...
                                get_string('forum-posts', 'format_culcourse')
                                );
                }
                ...
                if (!isset($hiddenfields['sendmessage'])) {
                    $links[] = html_writer::link(
                        ...
                        get_string('sendmessage', 'format_culcourse')
                        );
                }
