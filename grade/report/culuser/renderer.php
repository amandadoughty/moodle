...
require_once($CFG->dirroot . '/grade/report/user/renderer.php');

class gradereport_culuser_renderer extends gradereport_user_renderer {
    ...
    public function view_user_selector($userid, $userview) {
        ...
        $options = array(GRADE_REPORT_USER_VIEW_USER => get_string('otheruser', 'gradereport_culuser'),
                GRADE_REPORT_USER_VIEW_SELF => get_string('myself', 'gradereport_culuser'));
        $select = new single_select($url, 'userview', $options, $userview, null);

        $select->label = get_string('viewas', 'gradereport_culuser');
