...
/**
 * Unit tests for the gradereport_culuser implementation of the privacy API.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradereport_culuser_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences_single() {
        ...
        $this->setUser($user);
        set_user_preference('gradereport_culuser_view_user', 1, $user);
        ...
        $prefs = $writer->get_user_preferences('gradereport_culuser');
        $this->assertCount(1, (array) $prefs);
        $this->assertEquals(
            get_string('privacy:metadata:preference:gradereport_culuser_view_user', 'gradereport_culuser'),
            $prefs->gradereport_culuser_view_user->description
        );
        $this->assertEquals(get_string('yes'), $prefs->gradereport_culuser_view_user->value);
    }
}
