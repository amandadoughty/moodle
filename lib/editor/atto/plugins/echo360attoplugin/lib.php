...
const LTI_INSTRUCTOR_ROLES = array(LTI_INSTRUCTOR, LTI_TEACHER, LTI_EDITING_TEACHER, LTI_COURSE_CREATOR, LTI_MANAGER, LTI_ADMIN);
...
function atto_echo360attoplugin_params_for_js($elementid, $options, $fpoptions) {
  ...
  try {
    ...
    $params['disabled'] = (!has_capability('atto/echo360attoplugin:visible', $context) || !has_valid_roles($lti->get_roles_array()));
  } catch (Exception $e) {
 ...
 * @return bool
 */
function has_valid_roles($role_array) {
  $role_array = array_map('strtolower', $role_array);
  $has_valid_role = false;
  foreach($role_array as $role) {
    if(in_array($role, LTI_INSTRUCTOR_ROLES)){
      $has_valid_role = true;
      break;
    }
  }
