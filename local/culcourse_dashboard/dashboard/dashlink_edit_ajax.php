...
if ($action == SHOWHIDE) {
    if ($name) {
        echo local_culcourse_dashboard_quicklink_visibility($courseid, $name, $showhide, $lnktxt);
    } else {
        print_error('noname', 'local_culcourse_dashboard');
    }
} else if ($action == MOVE) {
    if (!empty($moveto) && !empty($copy) && !empty($name) && confirm_sesskey()) {
        $updated = local_culcourse_dashboard_dashlink_move($courseid, $name, $copy, $moveto);
