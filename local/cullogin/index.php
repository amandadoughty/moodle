...
$PAGE->set_url("$CFG->httpswwwroot/local/cullogin/index.php");
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /// Let's get them all set up.
        ...
        $urltogo = local_cullogin_get_return_url();
        ...
        redirect(new moodle_url(get_local_cullogin_url(), array('testsession'=>$USER->id)));
...
if (empty($SESSION->wantsurl)) {
    $SESSION->wantsurl = (array_key_exists('HTTP_REFERER',$_SERVER) &&
                          ...
                          $_SERVER["HTTP_REFERER"] != $CFG->wwwroot.'/' &&
                          $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot.'/local/cullogin/' &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/local/cullogin/?') !== 0 &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/local/cullogin/index.php') !== 0) // There might be some extra params such as ?lang=.
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/local/cullogin/index.php'));
}
...
if (isloggedin() and !isguestuser()) {
    ...
    $logout = new single_button(new moodle_url($CFG->httpswwwroot.'/local/cullogin/logout.php', array('sesskey'=>sesskey(),'loginpage'=>1)), get_string('logout'), 'post');
    $continue = new single_button(new moodle_url($CFG->httpswwwroot.'/local/cullogin/index.php', array('cancel'=>1)), get_string('cancel'), 'get');
