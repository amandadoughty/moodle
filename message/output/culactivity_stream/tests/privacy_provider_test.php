...
class message_culactivity_stream_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_delete_data_for_users() {
        ...
        $n2 = $this->create_notification($user2->id, $user1->id, $course1->id, $now + (8 * DAYSECS));
        $n3 = $this->create_notification($user2->id, $user3->id, $course1->id, $now + (8 * DAYSECS));
        $n4 = $this->create_notification($user1->id, $user4->id, $course1->id, $now + (8 * DAYSECS));

        // There should be three notifications.
        $this->assertEquals(4, $DB->count_records('message_culactivity_stream'));

        $user2context = context_user::instance($user2->id);

        $approveduserlist = new \core_privacy\local\request\approved_userlist($user2context, 'message_culactivity_stream',
                [$user2->id, $user3->id]);
        provider::delete_data_for_users($approveduserlist);

        // Confirm the user 4 data still exists.
        $notifications = $DB->get_records('message_culactivity_stream');
        $this->assertCount(1, $notifications);
        ksort($notifications);

        $notification = array_shift($notifications);
        $this->assertEquals($user1->id, $notification->userfromid);
        $this->assertEquals($user4->id, $notification->userid);
    }    
