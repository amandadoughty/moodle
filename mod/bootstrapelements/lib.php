...
function bootstrapelements_get_coursemodule_info($coursemodule) {
    ...
    if ($bootstrapelements = $DB->get_record('bootstrapelements', array('id' => $coursemodule->instance),
        ...
        if (!$bootstrapelements->name || (strpos($bootstrapelements->name, 'bootstrapelements') !== false)) {
            $bootstrapelements->name = $bootstrapelements->title;
             $DB->set_field('bootstrapelements', 'name', $bootstrapelements->name, array('id' => $bootstrapelements->id));
         }
 
        $cssid = "bootstrapelements" . $bootstrapelements->id;
        ...
        switch($bootstrapelements->bootstraptype) {
            case 0:
                $info->content = bootstrapelements_modal_outline($cssid, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon).
                        bootstrapelements_modal_button($cssid, $bootstrapelements->title,
                        $bootstrapelements->bootstrapicon);
            break;

            case 1:
                $info->content = bootstrapelements_toggle_outline($cssid, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon);
            break;

            case 2:
                $info->content = bootstrapelements_standard($cssid, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon);
            break;

            case 3:
                $info->content = bootstrapelements_blockquote($cssid, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon);
            break;
        }
