 ...
 * simplepa calculator.
 *
 * @package    peerwork_calculator_simplepa
 * @copyright  2019 Coventry University
 * @author     Frédéric Massart <fred@branchup.tech>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace peerworkcalculator_simplepa;
...
class calculator extends \mod_peerwork\peerworkcalculator_plugin {
    ...
    public function get_name() {
        return get_string('simplepa', 'peerworkcalculator_simplepa');
    }
    ...
    public function get_truncated() {
        if (is_null($this->truncated)) {
            $this->truncated =
                $this->get_config('simplepa_truncated') ?
                $this->get_config('simplepa_truncated') :
                get_config('peerworkcalculator_simplepa', 'simplepa_truncated');
        }
    ...
    public function calculate($grades, $groupmark, $noncompletionpenalty = 0, $paweighting = 1) {
        ...
        $upperboundary =
            $this->get_config('simplepa_upperboundary') ?
            $this->get_config('simplepa_upperboundary') :
            get_config('peerworkcalculator_simplepa', 'simplepa_upperboundary');
        $lowerboundary =
            $this->get_config('simplepa_lowerboundary') ?
            $this->get_config('simplepa_lowerboundary') :
            get_config('peerworkcalculator_simplepa', 'simplepa_lowerboundary');
    ...
    /**
     * Get the settings for simplepa calculator plugin
     *
    ...
    public function get_settings(\MoodleQuickForm $mform) {
        ...
        $default = $this->get_config('simplepa_truncated');
        if ($default === false) {
            // Apply the admin default if we don't have a value yet.
            $default = get_config('peerworkcalculator_simplepa', 'simplepa_truncated');
        }
        $name = get_string('truncated', 'peerworkcalculator_simplepa');
        $mform->addElement('selectyesno', 'simplepa_truncated', $name);
        $mform->addHelpButton('simplepa_truncated', 'truncated', 'peerworkcalculator_simplepa');
        $mform->setDefault('simplepa_truncated', $default);
        $mform->setType('simplepa_truncated', PARAM_INT);
        $mform->hideIf('simplepa_truncated', 'calculator', 'noteq', $this->get_type());

        $default = $this->get_config('simplepa_upperboundary');
        if ($default === false) {
            // Apply the admin default if we don't have a value yet.
            $default = get_config('peerworkcalculator_simplepa', 'simplepa_upperboundary');
        }
        $mform->addElement('hidden', 'simplepa_upperboundary', $default);
        $mform->setType('simplepa_upperboundary', PARAM_FLOAT);

        $default = $this->get_config('lowerboundary');
        if ($default === false) {
            // Apply the admin default if we don't have a value yet.
            $default = get_config('peerworkcalculator_simplepa', 'simplepa_lowerboundary');
        }
        $mform->addElement('hidden', 'simplepa_lowerboundary', $default);
        $mform->setType('simplepa_lowerboundary', PARAM_FLOAT);
    }
    ...
    public function save_settings(\stdClass $data) {
        if (empty($data->simplepa_truncated)) {
            $truncated = get_config('peerworkcalculator_simplepa', 'simplepa_truncated');
        } else {
            $truncated = $data->simplepa_truncated;
        }
        if (empty($data->simplepa_upperboundary)) {
            $upperboundary = get_config('peerworkcalculator_simplepa', 'simplepa_upperboundary');
        } else {
            $upperboundary = $data->simplepa_upperboundary;
        }
        if (empty($data->simplepa_lowerboundary)) {
            $lowerboundary = get_config('peerworkcalculator_simplepa', 'simplepa_lowerboundary');
        } else {
            $lowerboundary = $data->simplepa_lowerboundary;
        }
        $this->set_config('simplepa_truncated', $truncated);
        $this->set_config('simplepa_upperboundary', $upperboundary);
        $this->set_config('simplepa_lowerboundary', $lowerboundary);
