...
function theme_cul_boost_page_init(moodle_page $page) {
    ...
    $page->requires->js_call_amd('theme_cul_boost/replaceicons', 'init');
...
function theme_cul_boost_output_fragment_gradealert($args) {
    ...
    // if($cangrade) {
        ...
        if ($gradebookgrade->hidden){
            $o .= $OUTPUT->notification(get_string('gradehidden', 'theme_cul'), 'error hazard');
        } else {
            $o .= $OUTPUT->notification(get_string('gradenothidden', 'theme_cul'), 'error hazard');
        }
    // }
